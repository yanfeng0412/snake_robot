...
def calc_fitness(start_x, start_y, start_z,
    ...
    if fitness_option==1:
        ...
        # This formula yielded the stable walk in open loop
        fitness = 50*x_distance +50.0*x_vel- 100*abs(y_distance)+ up_time * 2
        if log_dis:
            log.info('[FIT] fitness = x_distance +vel - y_distance + up_time ')
    elif fitness_option == 7:
        # Fitness is the distance moved in the x direction (metres) + up_time (seconds)*0.5
        # This formula yielded the stable walk in open loop
        fitness = 80.0*x_vel - 0.3*np.sum(np.power(whole_pos_y, 2))
        if log_dis:
            log.info('[FIT] fitness =vel - whole_pos_y')
    elif fitness_option == 8:
        # Fitness is the distance moved in the x direction (metres) + up_time (seconds)*0.5
        # This formula yielded the stable walk in open loop
        fitness = 50*x_distance +50.0*x_vel- 0.3*np.sum(np.power(whole_pos_y, 2))+ up_time * 2
        if log_dis:
            log.info('[FIT] fitness = x_distance + x_vel - whole_pos_y + up_time')

    elif fitness_option == 9:
        # Fitness is the distance moved in the x direction (metres) + up_time (seconds)*0.5
        # This formula yielded the stable walk in open loop
        fitness = 50*x_distance +80.0*x_vel- 0.3*np.sum(np.power(whole_pos_y, 2))+ up_time * 2
        if log_dis:
            log.info('[FIT] fitness = x_distance + x_vel - whole_pos_y + up_time')

    elif fitness_option == 10:
        # Fitness is the distance moved in the x direction (metres) + up_time (seconds)*0.5
        # This formula yielded the stable walk in open loop
        fitness = 50*x_distance +50.0*x_vel- 0.1*np.sum(np.power(whole_pos_y, 2))+ up_time * 2
        if log_dis:
            log.info('[FIT] fitness = x_distance + x_vel - whole_pos_y + up_time')

    elif fitness_option == 11:
        # Fitness is the distance moved in the x direction (metres) + up_time (seconds)*0.5
        # This formula yielded the stable walk in open loop
        fitness = 80*x_distance +50.0*x_vel- 0.3*np.sum(np.power(whole_pos_y, 2))+ up_time * 2
        if log_dis:
            log.info('[FIT] fitness = x_distance + x_vel - whole_pos_y + up_time')

    elif fitness_option == 12:
        # Fitness is the distance moved in the x direction (metres) + up_time (seconds)*0.5
        # This formula yielded the stable walk in open loop
        fitness = 50*x_distance +80.0*x_vel- 0.1*np.sum(np.power(whole_pos_y, 2))+ up_time * 2
        if log_dis:
            log.info('[FIT] fitness = x_distance + x_vel - whole_pos_y + up_time')
    return fitness
