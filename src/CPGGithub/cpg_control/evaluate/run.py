...
from my_envs.base.command_generator import *
import gym
...
def main(args):
    ...
    if args.play:
        ...
        for i in range(max_step):
            ...
            v_e.append(info[0]['velocity_base'])
            c_command.append(info[0]['commands'])
            xyz.append(info[0]['obs'][:3])
        ...
        plot_velocity_curve(v_e, c_command, max_step, dt=0.01, save_plot_path=args.save_plot_path)
        plot_position_time(xyz, max_step, dt=0.01, save_plot_path=args.save_plot_path)
        plot_traj_xy(xyz, max_step, dt=0.01, save_plot_path=args.save_plot_path)
...
#
#     next_obs, reward, done, arg = env.step(action)
#
#
#
#     obs = next_obs
#
#     v_e.append(arg['velocity_base'])
#     c_command.append(arg['commands'])
#     xyz.append(obs[:3])
#
...
# xyz = np.array(xyz)
#
# plot_velocity_curve(v_e, c_command, max_step, dt = 0.01)
# plot_position_time(xyz, max_step, dt = 0.01)
# plot_traj_xy(xyz, max_step, dt = 0.01)